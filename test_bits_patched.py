...
with context: 
    ...
    hidden_states = torch.randn(1, 5, 4096, device=f"cuda:{dist.get_rank()}", dtype=torch.bfloat16)

    # query_states_before = q_proj(hidden_states)
    ...
    #     print(f"{query_states_before=}")
    #     print(f"{query_states=}")
    #     print(f"{query_states_before.size()=}")
